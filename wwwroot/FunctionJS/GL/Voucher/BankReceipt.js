...
$(document).on("click", '#btn_new', function () {
    ...
    txtchqdat.find("input").val(CurrentDate);
});

$(function () {
    txtdat.datetimepicker({ format: 'DD/MMM/YYYY' });
    txtchqdat.datetimepicker({ format: 'DD/MMM/YYYY' });
});
                ...
                if (response["data"] != null) {
                    detailsTableBody = $("#Table_View").dataTable({
                        ...
                        columns: [
                            ...
                            {
                                data: 'amount',
                                render: function (data, type, row) { return accounting.formatNumber(data, 2) }
                            },
                            { data: 'status' },
                            { data: 'type' }
                        ],
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (response.statusCode == 200) {
                        ...
                        txtdat.find("input").val(moment(response["data"]["transactionDate"]).format("DD/MMM/YYYY"));
                        txtchqdat.find("input").val(moment(response["data"]["date"]).format("DD/MMM/YYYY"));
                        $('#txt_chqno').val(response["data"]["chqNo"]);
                        ...
                        $('#txt_pto_master').val(response["data"]["paidReceived"]);
                        $('#txt_ExchangeRate').val(response["data"]["exchangeRate"]);
                        $('#txt_rmk').val(response["data"]["remarks"]);


                        if (!response["data"]["onlineCheck"]) {
                            $('#ck_Online').iCheck('uncheck'); //To uncheck the radio button
                        }
                        else if (response["data"]["onlineCheck"]) {
                            $('#ck_Online').iCheck('check'); //To check the radio button
                        }
                        //Detail Start
                        var productItem;
                        var _Detail = response["data"]["voucherDetailViewByIdList"];
                        for (var row_cnt = 0; row_cnt < _Detail.length; row_cnt++) {
                            if (parseFloat(_Detail[row_cnt]["debitAmount"]) > 0) {
                                $('#txt_nar_mas').val(_Detail[row_cnt]["naration"]);

                                _Account_No = _Detail[row_cnt]["accountNo"];
                                _Account_Name = _Detail[row_cnt]["accountName"];
                                $('#txt_acc_mas').val(_Account_No); // Select the option with a value of '1'
                                $('#txt_acc_mas').trigger('change'); // Notify any JS components that the value changed

                            }
                            if (parseFloat(_Detail[row_cnt]["creditAmount"]) > 0) {
                                productItem += '<tr>' +
...
$("#txt_cr_amt").keyup(function (e) {
    if (e.keyCode === 13) {
        ...
        var productItem = '<tr><td><a data-itemId="0" href="#" class="deleteItem">Remove</a></td><td>' + account.select2('data').text + '</td><td>' + narration + '</td><td>' + accounting.formatNumber(credit, 2) + '</td><td hidden>' + account.select2('data').id + '</td></tr>';
        detailsTableBody.append(productItem);
        discon_det();
    }
});
...
function dgcal() {
    ...
    for (var i = 0; i < rows_create.length; i++) {
        ...
        amt += parseFloat(accounting.unformat($(columns[3]).html()));
    }
    $("#txt_dr_amt").html(accounting.formatNumber(amt, 2));
}
...
function FillMasterAccount() {
    $("#txt_acc_mas").select2({
        ...
        initSelection: function (element, callback) {
            var data = { "id": _Account_No, "text": _Account_Name };
            callback(data);
        },
...
//     var txtnarmaster = $("#txt_nar_mas").val();
//     var txtdramt = $("#txt_dr_amt").html();
//     var detail_record = txtaccmaster.select2('data').text + "|" + txtnarmaster + "|" + txtdramt + "|0";
...
//         if (detail_record == "") {
//             detail_record = $(columns[1]).html() + "|" + $(columns[2]).html() + "|0|" + $(columns[3]).html();
//         }
//         else {
//             detail_record += "|" + $(columns[1]).html() + "|" + $(columns[2]).html() + "|0|" + $(columns[3]).html();
//         }
...
function ckvalidation() {
    ...
    var txtdPaidReceived = $("#txt_pto_master").val();

    var txtAccountMaster = $("#txt_acc_mas");
    var txtNarrationMaster = $("#txt_nar_mas").val();
    var txtAmountMaster = $("#txt_dr_amt").html();

    var txtDat1 = moment(txtdat.find("input").val()).format("YYYY-MM-DD");
    var txtChqDat1 = moment(txtchqdat.find("input").val()).format("YYYY-MM-DD");
    ...
    var ckOnline = $("#ck_Online").iCheck('Update')[0].checked;
    ...
    else if (txtAccountMaster.val() == '') {
        ck = 1;
        _Error = 'Please Select Account';

    }
    else if (txtNarrationMaster == '') {
        ck = 1;
        _Error = 'Narration cannot be blank';

    }
    else if (txtdPaidReceived == '') {
        ck = 1;
        _Error = 'Received from cannot be blank';

    }
    else if (txtChqNo == '') {
        ck = 1;
        _Error = 'Chq no cannot be blank';
    ...
    detail_record.push({
        "accountNo": txtAccountMaster.select2('data').id,
        "naration": txtNarrationMaster,
        "debitAmount": accounting.unformat(txtAmountMaster),
        "creditAmount": 0,
    })
    for (var i = 0; i < rows_create.length; i++) {
        columns = $(rows_create[i]).find('td');
        detail_record.push({
            "accountNo": $(columns[4]).html(),
            "naration": $(columns[2]).html(),
            "debitAmount": 0,
            "creditAmount": accounting.unformat($(columns[3]).html()),
        })
    }
    _cre = JSON.stringify({
        "no": txtNo,
...
function discon() {
    ...
    _Account_No = "0";
    _Account_Name = "";
    $('#ck_Online').iCheck('update')[0].uncheck;
    txtno.html('');
    txtid.html('');
