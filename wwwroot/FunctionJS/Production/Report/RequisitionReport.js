...
function printReport() {
    ...
    detailsTable.empty();
    $.ajax({
        url: ApiForm + '/v1/ProductionReport/GetRequisitionProduction',
        type: "Post",
        ...
        success: function (response) {
            if (response.statusCode == 200) {
                ...
                const qoutno_1 = response["data"]["requisitionProductionMasters"];
                ...
                for (var row_cnt = 0; row_cnt < qouno.length; row_cnt++) {
                    const _transaction = response["data"]["requisitionProductionMasters"].filter(d => d.id == qouno[row_cnt]["_id"]);
                    var _columnno = 10;
                    ...
                    rowdata += '<thead>' +
                        '<tr>' +
                        '<th colspan="10" style="text-align:center;font-size:14pt;font-weight:bold;">' + response["data"]["companyName"] + '</th>' +
                        '</tr>' +
                        '<tr>' +
                        '<th colspan="10" style="text-align:center;font-variant-caps:all-petite-caps;font-size:12pt;font-weight:bold;">' + response["data"]["headingName"] + '</th>' +
                        '</tr>' +
                        '<tr>' +
                        '<th colspan="10" style="text-align:right;font-variant-caps:all-petite-caps;font-size:6pt;font-weight:bold;">' + today + '</th>' +
                        '</tr>' +
                        '</thead>';
                    rowdata += '<thead>' +
                        '<tr>' +
                    ...
                    rowdata += '</thead>';
                    var Detailsrecord = _transaction[0]["requisitionProductionDetails"];
