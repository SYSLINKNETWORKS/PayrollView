...
var ComponentsDropdowns = function () {
    var handleSelect2 = function () {
        ...
        FILLUSERS(); //Fill Select 2 of Users       
    }
...
function FILLUSERS() {
    ...
    $("#txt_user").select2({
        ...
        ajax: { // instead of writing the function to execute the request we use Select2's convenient helper
            url: apiUrl + '/api/Auth/v1/LOVServices/GetUsers',
            type: "Get",
        ...
        initSelection: function (element, callback) {
            var data = { "id": _User_ID, "text": _UserName_Name };
            callback(data);
        },
...
$('#txt_user').on("select2-selected", function (e) {
    _User_ID = $("#txt_user").select2('data').id;
    _UserName_Name = $("#txt_user").select2('data').text;

    //Fill Users in table 
    var _UsersTable = $("#UsersTable tbody");
    var _Row = '<tr>' +
        '<td><a data-itemId="0" href="#" class="deleteItem">Remove</a></td>' +
        '<td hidden>' + _User_ID + '</td>' +
        '<td>' + _UserName_Name + '</td>';
    _Row += '</tr>';
    _UsersTable.append(_Row);
});

$('#txt_user').on("select2-removed", function (e) {
    _User_ID = "0";
...
function Onload() {
    var tbl_row_cnt = 1;
    $.ajax({
        url: ApiForm + '/ProductionDepartment',
        type: "Get",
        contentType: "application/json",
        dataType: "json",
...
$('table').on('click', '.btn-edit', function (e) {
    ...
    Swal.fire({
        ...
        if (result.value) {
            $.ajax({
                url: ApiForm + '/ProductionDepartment/GetProductionDepartmentById',
                type: "Get",
                contentType: "application/json",
                dataType: "json",
                beforeSend: function (xhr) {
                ...
                success: function (response) {
                    if (response.statusCode == 200) {
                        ...
                        if (response["data"]["_ProductionDepartmentUserViewByIdList"] != null) {
                            var _UserTable = $("#UsersTable tbody");
                            _UserTable.empty();
                            for (var user_row_cnt = 0; user_row_cnt < response["data"]["_ProductionDepartmentUserViewByIdList"].length; user_row_cnt++) {
                                UsersId = response["data"]["_ProductionDepartmentUserViewByIdList"][user_row_cnt]["userid"];
                                UserName = response["data"]["_ProductionDepartmentUserViewByIdList"][user_row_cnt]["userName"];

                                var _Row = '<tr>' +
                                    '<td><a data-itemId="0" href="#" class="deleteItem">Remove</a></td>' +
                                    '<td hidden>' + UsersId + '</td>' +
                                    '<td>' + UserName + '</td>';
                                _Row += '</tr>';
...
function ckvalidation() {
    ...
    var rows_create = $("#UsersTable tbody >tr");
    var detail_record = [];

    for (var i = 0; i < rows_create.length; i++) {
        columns = $(rows_create[i]).find('td');

        var UsersId = $(columns[1]).html().trim();
        detail_record.push({
            "userid": UsersId,
        })
    }
