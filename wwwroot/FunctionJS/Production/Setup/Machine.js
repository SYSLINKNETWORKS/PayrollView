...
var ComponentsDropdowns = function () {
    var handleSelect2 = function () {
        ...
        FILLPRODUCTIONDEPARTMENT(); //Fill Select 2 of Users       
    }
...
function FILLPRODUCTIONDEPARTMENT() {
    $("#txt_ProductionperDepartment").select2({
...
$('#txt_ProductionperDepartment').on("select2-selected", function (e) {
    _productioDepartment_Id = $("#txt_ProductionperDepartment").select2('data').id;
});

$('#txt_ProductionperDepartment').on("select2-removed", function (e) {
    _productioDepartment_Id = "0";
...
function discon() {
    ...
    btnsav.hide();
    btnupd.hide();
    $('#txt_ProductionperDepartment').html('');
    $('#txt_ProductionperDepartment').select2('val', '');
                ...
                if (response["data"] != null) {
                    ...
                    detailsTableBody = $("#Table_View").dataTable({
                        ...
                        columns: [
                            ...
                            //             return 'Printing ';
                            //         }
                            //         else if (data == 2) { return 'UV'; }
                            //         else if (data == 3) { return 'Lamination'; }
                            //         else if (data == 4) { return 'Foiling'; }
                            //         else if (data == 5) { return 'Die-Cutting'; }
                            //         else if (data == 6) { return 'Pasting'; }
                            //         else if (data == 7) { return 'Machine'; }
                            //         else if (data == 8) { return 'Sublet'; }
                            //         else if (data == 9) { return 'Packing'; }
                            //     }
                            // },
                            {
...
$('table').on('click', '.btn-edit', function (e) {
    ...
    Swal.fire({
        ...
        if (result.value) {
            $.ajax({
                url: ApiForm + '/Machine/GetMachineById',
                type: "Get",
                contentType: "application/json",
                dataType: "json",
                beforeSend: function (xhr) {
                ...
                success: function (response) {
                    if (response.statusCode == 200) {
                        ...
                        //$('#txt_departement').val(response["data"]["department"]);
                        $('#txt_ProductionperHour').val(response["data"]["productionPerHour"]);
                        if (!response["data"]["active"]) {
                            $('#ck_act').iCheck('uncheck');
                        } else { $('#ck_act').iCheck('check'); }
                        ...
                        $('#txt_ProductionperDepartment').val(_productioDepartment_Id); // Select the option with a value of '1'
                        $('#txt_ProductionperDepartment').trigger('change'); // Notify any JS components that the value changed
...
function ckvalidation() {
    ...
    var txt_Production = $("#txt_ProductionperDepartment").select2('data').id;
    var department = $('#txt_ProductionperDepartment');
    var Production_hr = $('#txt_ProductionperHour');
    ck_active = $("#ck_act").iCheck('Update')[0].checked;
    ...
    if (Production_hr.val() == '') {
        ck = 1;
        _Error = 'Please Enter Production hour';
        Production_hr.focus();
    }
    ...
    if (Boolean(ck)) {
        ...
        _cre = JSON.stringify({
            "ID": txtid,
            "name": txtname.val(),
            "productionDepartmentId": txt_Production,
            "ProductionPerHour": Production_hr.val(),
            "active":ck_active,
            "type": "U",
            "menu_Id": _menuid
        });
