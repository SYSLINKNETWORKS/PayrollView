        ...
        ajax: { // instead of writing the function to execute the request we use Select2's convenient helper
            ...
            transport: function (params) {
                params.beforeSend = function (request) {
                    ...
                    request.setRequestHeader("DepartmentSequence", 9);
                };
                return $.ajax(params);
    ...
    $("#txt_machine").select2({
        ...
        ajax: { // instead of writing the function to execute the request we use Select2's convenient helper
            ...
            transport: function (params) {
                params.beforeSend = function (request) {
                    request.setRequestHeader("Authorization", 'Bearer ' + strkey);
                    request.setRequestHeader("ProductionDepartmentName", 'Packing');
                };
                return $.ajax(params);
            }, data: function (term, page) {
                return {
...
$('table').on('click', '.btn-edit', function (e) {
    ...
    Swal.fire({
        ...
        if (result.value) {
            $.ajax({
                url: ApiForm + '/PackingMaster/GetPackingById',
                type: "Get",
                contentType: "application/json",
                dataType: "json",
                beforeSend: function (xhr) {
                ...
                success: function (response) {
                    if (response.statusCode == 200) {
                        ...
                        _detailsTable.empty();
                        if (response["data"]["packingDetailViewByIdLists"] != null) {
                            for (var user_row_cnt = 0; user_row_cnt < response["data"]["packingDetailViewByIdLists"].length; user_row_cnt++) {
                                _MachineId = response["data"]["packingDetailViewByIdLists"][user_row_cnt]["machineId"];
                                _MachineName = response["data"]["packingDetailViewByIdLists"][user_row_cnt]["machineName"];
                                _Time = moment(response["data"]["packingDetailViewByIdLists"][user_row_cnt]["time"]).format("HH:mm:ss");
                                _productionqty = response["data"]["packingDetailViewByIdLists"][user_row_cnt]["qty"];
                                _wastageqty = response["data"]["packingDetailViewByIdLists"][user_row_cnt]["wastageQty"];
                                _WarehouseId = response["data"]["packingDetailViewByIdLists"][user_row_cnt]["warehouseId"];
                                _WarehouseName = response["data"]["packingDetailViewByIdLists"][user_row_cnt]["warehouseName"];

                                var _Row = '<tr>' +
                                    '<td><a data-itemId="0" href="#" class="deleteItem">Remove</a></td>' +
                                    '<td>' + _Time + '</td>' +
                                    '<td style="display:none;">' + _MachineId + '</td>' +
                                    '<td>' + _MachineName + '</td>' +
                                    '<td>' + _productionqty + '</td>' +
                                    '<td>' + _wastageqty + '</td>' +
                                    '<td style="display:none;">' + _WarehouseId + '</td>' +
                                    '<td>' + _WarehouseName + '</td>';
                                _Row += '</tr>';
...
function ckvalidation() {
    ...
    _cre = JSON.stringify({
        ...
        "type": "U",
        "menu_Id": _menuid,
        "PackingDetailViewModels": detail_record,
    });
...
function AddDetail() {
    ...
    var _Row = '<tr>' +
        '<td><a data-itemId="0" href="#" class="deleteItem">Remove</a></td>' +
        '<td>' + _ddl_time.val() + '</td>' +
        '<td style="display:none;">' + _txt_machine.select2('data').id + '</td>' +
        '<td>' + _txt_machine.select2('data').text + '</td>' +
        '<td>' + _txt_productionqty.val() + '</td>' +
        '<td>' + _txt_wastageqty.val() + '</td>' +
        '<td style="display:none;">' + _txt_warehouse.select2('data').id + '</td>' +
        '<td >' + _txt_warehouse.select2('data').text + '</td>';
    _Row += '</tr>';
    _detailsTable.append(_Row);
    //_ddl_time.val('');
    _txt_machine.val('');
