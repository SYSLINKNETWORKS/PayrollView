        ...
        ajax: { // instead of writing the function to execute the request we use Select2's convenient helper
            ...
            transport: function (params) {
                params.beforeSend = function (request) {
                    ...
                    request.setRequestHeader("DepartmentSequence", 6);
                };
                return $.ajax(params);
    ...
    $("#txt_machine").select2({
        ...
        ajax: { // instead of writing the function to execute the request we use Select2's convenient helper
            ...
            transport: function (params) {
                params.beforeSend = function (request) {
                    request.setRequestHeader("Authorization", 'Bearer ' + strkey);
                    request.setRequestHeader("ProductionDepartmentName", 'Pasting');
                };
                return $.ajax(params);
            }, data: function (term, page) {
                return {
...
$('table').on('click', '.btn-edit', function (e) {
    ...
    Swal.fire({
        ...
        if (result.value) {
            $.ajax({
                url: ApiForm + '/PastingMaster/GetPastingById',
                type: "Get",
                contentType: "application/json",
                dataType: "json",
                beforeSend: function (xhr) {
                ...
                success: function (response) {
                    if (response.statusCode == 200) {
                        ...
                        _detailsTable.empty();
                        if (response["data"]["pastingDetailViewByIdLists"] != null) {
                            for (var user_row_cnt = 0; user_row_cnt < response["data"]["pastingDetailViewByIdLists"].length; user_row_cnt++) {
                                _MachineId = response["data"]["pastingDetailViewByIdLists"][user_row_cnt]["machineId"];
                                _MachineName = response["data"]["pastingDetailViewByIdLists"][user_row_cnt]["machineName"];
                                _Time = moment(response["data"]["pastingDetailViewByIdLists"][user_row_cnt]["time"]).format("HH:mm:ss");
                                _productionqty = response["data"]["pastingDetailViewByIdLists"][user_row_cnt]["qty"];
                                _wastageqty = response["data"]["pastingDetailViewByIdLists"][user_row_cnt]["wastageQty"];
...
function ckvalidation() {
    ...
    _cre = JSON.stringify({
        ...
        "type": "U",
        "menu_Id": _menuid,
        "PastingDetailViewModels": detail_record,
    });
