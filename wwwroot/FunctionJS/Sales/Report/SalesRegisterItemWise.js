...
function printReport() {
    ...
    $.ajax({
        ...
        url: apiUrl + '/Sales/Report/SaleReport/SalesRegisterItemWise',
        type: "Post",
        ...
        success: function (response) {
            if (response[0].status == 1) {
                ...
                rowdata += '<thead>' +
                    ...
                    '</tr>' +
                    '<tr>' +
                    '<th colspan="16" style="text-align:center;font-variant-caps:all-petite-caps;font-size:12pt;font-weight:bold;">Sales Register Item Wise</th>' +
                    '</tr>' +
                    '<tr>' +
                ...
                for (var row_cnt_brand = 0; row_cnt_brand < _Brand.length; row_cnt_brand++) {
                    ...
                    for (var row_cnt_item = 0; row_cnt_item < _Item.length; row_cnt_item++) {
                        ...
                        for (var row_cnt_1 = 0; row_cnt_1 < _transaction.length; row_cnt_1++) {
                            ...
                            rowdata += '<tr>' +
                               ...
                               // '<td style="text-align:center;">' + _transaction[row_cnt_1]["Supplier_Bill"] + '</td>' +
                                '<td width=2000px>' + _transaction[row_cnt_1]["Customer"] + '</td>' +
                                '<td style="text-align:center;">' + accounting.formatNumber(_transaction[row_cnt_1]["Qty"], 0) + '</td>' +
                                '<td style="text-align:right;">' + accounting.formatNumber(_transaction[row_cnt_1]["Rate"], 2) + '</td>' +
                        ...
                        rowdata += '<tr  style="color:red;">' +
                            ...
                            '<td style="text-align:right;">' + accounting.formatNumber(avgrat, 2) + '</td>' +
