...
function printReport() {
    ...
    $.ajax({
        ...
        url: apiUrl + '/Sales/Report/SaleReport/CustomerOutstanding/' + credat,
        type: "Post",
            ...
            if (response[0].status == 1) {
                ...
                for (var row_cnt = 0; row_cnt < vchno.length; row_cnt++) {
                    ...
                    rowdata += '<thead>' +
                        ...
                        '</tr>' +
                        '<tr>' +
                        '<th colspan="8" style="text-align:center;font-variant-caps:all-petite-caps;font-size:12pt;font-weight:bold;">Customer Outstanding</th>' +
                        '</tr>' +
                        '<tr>' +
                        '<th colspan="8" style="text-align:right;font-variant-caps:all-petite-caps;font-size:7pt;font-weight:bold;">' + today + '</th>' +
                        '</tr>';

                    rowdata += '<tr style="text-align:center;"><th>S.No</th><th>Customer</th><th>Amount</th></tr>';
                    rowdata += '</thead>';
                    ...
                    for (var row_cnt_1 = 0; row_cnt_1 < _transaction.length; row_cnt_1++) {
                        ...
                        rowdata += '<tr>' +
                            ...
                            '<td width=2000px><a href="#" onclick=window.open("' + apiUrl_View + '/GL_Report/GeneralLedger?1&' + sessid_gl + '&' + datfrm + '&' + datto + '","_blank")>' + _transaction[row_cnt_1]["Customer"] + '</a></td>' +
                            '<td style="text-align:right;">' + accounting.formatNumber(_transaction[row_cnt_1]["balance"], 0) + '</td>' +
                            '</tr>';
