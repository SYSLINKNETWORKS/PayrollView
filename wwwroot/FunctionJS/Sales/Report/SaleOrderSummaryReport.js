...
function printReport() {
    ...
    detailsTable = $("#div_Saleorder");
    detailsTable.empty();
    $.ajax({
        url: apiUrl + '/api/Sales/v1/Sales/SaleOrderSummaryReport',
        type: "Post",
            ...
            if (response.statusCode == 200) {
                ...
                for (var row_cnt = 0; row_cnt < response["data"].length; row_cnt++) {
                    ...
                    rowdata += '<tr>' +
                        ...
                        '<td >' + moment(response["data"][row_cnt]["deliveryDate"]).format("DD-MMM-YYYY") + '</td>' +
                        '<td>' + response["data"][row_cnt]["customerName"] + '</td>' +
                        '<td>' + response["data"][row_cnt]["poNo"] + '</td>' +
                        '<td >' + response["data"][row_cnt]["itemName"] + '</td>' +
                        '<td>' + response["data"][row_cnt]["uom"] + '</td>' +
                        '<td> ' + accounting.format(response["data"][row_cnt]["qty"], 0) + '</td>' +
                        '<td> ' + accounting.format(response["data"][row_cnt]["orderQty"], 0) + '</td>' +
                        '<td ' + _styleBalance + ' > ' + accounting.format(response["data"][row_cnt]["balanceQty"], 0) + '</td>' +
