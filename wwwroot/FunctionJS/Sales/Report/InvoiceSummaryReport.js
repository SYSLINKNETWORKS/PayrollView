...
function printReport() {
    ...
    detailsTable = $("#div_invoice");
    detailsTable.empty();
    $.ajax({
        url: ApiForm + '/v1/Sales/InvoiceSummaryReport',
        type: "Post",
        ...
        success: function (response) {
            if (response.statusCode == 200) {
                ...
                rowdata += '</thead>';
                rowdata += '<body>';
                var _NetAmount = 0, _GrossAmount = 0;
                for (var row_cnt = 0; row_cnt < response["data"].length; row_cnt++) {
                    ...
                    rowdata += '<tr>' +
                        ...
                        '<td >' + accounting.format(response["data"][row_cnt]["excludingGSTAmount"], 0) + '</td>' +
                        '<td >' + accounting.format(response["data"][row_cnt]["includingGSTAmount"], 0) + '</td>' +
                        '<td colspan="2" ' + _styleStatus + ' > ' + response["data"][row_cnt]["status"] + ' </td>' +
                        '</tr>';
                    _NetAmount += parseFloat(response["data"][row_cnt]["excludingGSTAmount"]);
                    _GrossAmount += parseFloat(response["data"][row_cnt]["includingGSTAmount"]);
                ...
                rowdata += '<tfooter>';
                rowdata += '<tr style="font-weight:bold;">' +
                    '<th style="font-weight:bold;text-align:right;" colspan=13>Total</th>' +
                    '<th>' + accounting.format(_NetAmount, 0) + ' </th>' +
                    '<th>' + accounting.format(_GrossAmount, 0) + ' </th>' +
                    '<th></th>' +
                    '</tr>';
                rowdata += '</tfooter>';
                rowdata += '</table>';
