...
function Onload() {
    var tbl_row_cnt = 1;
    $.ajax({
        url: ApiForm + '/Designation?_MenuId=' + _menuid,
        type: "Get",
...
function savrec() {
    Swal.fire({
        ...
        if (result.value) {
            ...
            var ckdirector = $('#ck_director').iCheck('update')[0].checked;
            var cksalesman = $('#ck_salesman').iCheck('update')[0].checked;
            $.ajax({
                url: ApiForm + '/Designation',
                type: "Post",
                contentType: "application/json",
                dataType: "json",
                data: JSON.stringify({
    ...
    Swal.fire({
        ...
        if (result.value) {
            $.ajax({
                ...
                url: ApiForm + '/Designation/GetDesignationById',
                type: "Get",
                contentType: "application/json",
                dataType: "json",
                beforeSend: function (xhr) {
                ...
                success: function (response) {
                    ...
                    if (response.statusCode == 200) {
                        ...
                        if (!response["data"]["salesman"]) {
                            $('#ck_salesman').iCheck('uncheck');//To uncheck the radio button
                        }else if (response["data"]["salesman"])
                        { $('#ck_salesman').iCheck('check'); }

                        if (!response["data"]["director"]) {
                            $('#ck_director').iCheck('uncheck'); //To uncheck the radio button
                        }else if (response["data"]["director"])
                        { $('#ck_director').iCheck('check'); }
