...
function Onload() {
    var tbl_row_cnt = 1;
    $.ajax({
        url: ApiForm + '/Allowance?_MenuId=' + _menuid,
        type: "Get",
...
function savrec() {
    Swal.fire({
        ...
        if (result.value) {
            ...
            var txt_amnt = $('#txt_amount').val();
            cktus = $('#ck_fix').iCheck('update')[0].checked;

            $.ajax({
                url: ApiForm + '/Allowance',
                type: "Post",
                contentType: "application/json",
                dataType: "json",
                data: JSON.stringify({
    ...
    Swal.fire({
        ...
        if (result.value) {
            $.ajax({
                ...
                url: ApiForm + '/Allowance/GetAllowanceById',
                type: "Get",
                contentType: "application/json",
                dataType: "json",
                beforeSend: function (xhr) {
                ...
                success: function (response) {
                    ...
                    if (response.statusCode == 200) {
                        ...
                        if (!response["data"]["fix"]) {
                            $('#ck_fix').iCheck('uncheck');
                        } else { $('#ck_fix').iCheck('check'); }
