...
function printReport() {
    detailsTable = $("#div_singledayatt");
    detailsTable.empty();
    $.ajax({
        url: apiUrl + '/api/Payroll/v1/PayrollReport/SingleDayAttandance',
        type: "Post",
        ...
        success: function (response) {
            if (response.statusCode == 200) {
                ...
                rowdata += '<tr><th style="font-weight:bold;text-align:center;" colspan=21>SINGLE DAY ATTENDANCE</th></tr>';
                ...
                const employee_1 = response.data["singleDayAttandances"];
                // var employee = [];
                ...
                for (var row_cnt = 0; row_cnt < employee.length; row_cnt++) {
                    ...
                    const employeeFilter = response.data["singleDayAttandances"].filter(d => d.employeeId == employee[row_cnt]._id);
                    for (var row_cnt_filter = 0; row_cnt_filter < employeeFilter.length; row_cnt_filter++) {

                        rowdata += '<tr style="font-size:7pt;">' +
                            '<td>' + sno++ + '</td>' +
                            '<td>' + employeeFilter[row_cnt_filter]["employeeNo"] + '</td>' +
                            '<td>' + employeeFilter[row_cnt_filter]["employeeName"] + '</td>' +
                            '<td>' + moment(employeeFilter[row_cnt_filter]["dailyDate"]).format("DD-MMM-YYYY") + '</td>' +
