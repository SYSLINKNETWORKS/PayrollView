...
function printReport() {
    detailsTable = $("#div_salaryincrement");
    detailsTable.empty();
    $.ajax({
        url: ApiForm + '/v1/PayrollReport/SalaryIncrement',
        type: "Post",
        ...
        success: function (response) {
            if (response.statusCode == 200) {
                ...
                rowdata += '<tr><th style="font-weight:bold;text-align:center;" colspan=21>Salary Increment Summary</th></tr>';
                ...
                rowdata += '</thead>';
                const Date_1 = response.data["salaryIncrementReportList"];
                var _Date = [];
                ...
                for (var dt_cnt = 0; dt_cnt < _Date.length; dt_cnt++) {
                    ...
                    const department_1 = response.data["salaryIncrementReportList"].filter(d => d.date == _Date[dt_cnt]._dt);
                    var department = [];
                    ...
                    for (var department_cnt = 0; department_cnt < department.length; department_cnt++) {
                        ...
                        const employee_1 = response.data["salaryIncrementReportList"].filter(d => d.date == _Date[dt_cnt]._dt && d.departmentName == department[department_cnt]._nam);
                        var employee = [];
                        ...
                        for (var row_cnt = 0; row_cnt < employee.length; row_cnt++) {
                            const employeeFilter = response.data["salaryIncrementReportList"].filter(d => d.employeeId == employee[row_cnt]._id);
                            for (var row_cnt_filter = 0; row_cnt_filter < employeeFilter.length; row_cnt_filter++) {
                                rowdata += '<tr style="font-size:7pt;">' +
                                    ...
                                    '<td>' + accounting.format(employeeFilter[row_cnt_filter]["previousSalary"]) + '</td>' +
                                    '<td>' + employeeFilter[row_cnt_filter]["incrementPercentage"] + '</td>' +
                                    '<td>' + accounting.format(employeeFilter[row_cnt_filter]["incrementAmount"],0) + '</td>' +
