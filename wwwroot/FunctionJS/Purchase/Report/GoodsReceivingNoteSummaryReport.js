...
function printReport() {
    ...
    detailsTable.empty();
    $.ajax({
        url: ApiForm + '/v1/PurchaseReport/GoodsReceivingNoteSummaryReport',
        type: "Post",
            ...
            if (response.statusCode == 200) {
                ...
                for (var row_cnt = 0; row_cnt < _FilterData.length; row_cnt++) {
                    ...
                    rowdata += '<tr>' +
                        ...
                        '<td >' + _FilterData[row_cnt]["poNo"] + '</td>' +
                        '<td >' + _FilterData[row_cnt]["supplierName"] + '</td>' +
                        '<td>' + _FilterData[row_cnt]["supplierDCNo"] + '</td>' +
                        '<td> ' + moment(_FilterData[row_cnt]["supplierDCDate"]).format("DD-MMM-YYYY") + '</td>' +
