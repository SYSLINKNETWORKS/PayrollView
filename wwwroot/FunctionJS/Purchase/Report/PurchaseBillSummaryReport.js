...
function printReport() {
    ...
    var _taxableType = _crestr["taxableType"];
    var _processType = _crestr["processType"];

    detailsTable = $("#div_PurchaseBill");
    detailsTable.empty();
    $.ajax({
        url: ApiForm + '/v1/PurchaseReport/BillSummaryReport',
        type: "Post",
        ...
        success: function (response) {
            if (response.statusCode == 200) {
                ...
                if (_taxableType != 'All') {
                    _FilterData = response["data"].filter(d => d.taxable== _taxableType);
                }
                ...
                for (var row_cnt = 0; row_cnt < _FilterData.length; row_cnt++) {
                    ...
                    rowdata += '<tr>' +
                        ...
                        '<td >' + _FilterData[row_cnt]["supplierBillNO"] + '</td>' +
                        '<td> ' + accounting.format(_FilterData[row_cnt]["amount"]) + '</td>' +
                        '<td>' + _FilterData[row_cnt]["taxable"] + '</td>' +
