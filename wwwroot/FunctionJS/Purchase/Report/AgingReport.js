...
function printReport() {
    detailsTable = $("#div_Aging");
    detailsTable.empty();
    $.ajax({
        url: ApiForm + '/v1/PurchaseReport/AgingReport',
        type: "Post",
        ...
        success: function (response) {
            if (response.statusCode == 200) {
                ...
                const _supplierRecord = response["data"];
                var _supplierArray = [];
                for (i = 0; i < _supplierRecord.length; i++) {

                    if (_supplierArray.findIndex(x => x._supplierName == _supplierRecord[i].supplierName) == -1) {
                        _supplierArray.push({ _supplierName: _supplierRecord[i].supplierName, _creditDays: _supplierRecord[i].creditDays });
                    }
                }
                _supplierArray.sort((a, b) => (a._supplierName > b._supplierName) ? 1 : -1);

                for (var row_cnt = 0; row_cnt < _supplierArray.length; row_cnt++) {
                    ...
                    rowdata += '<thead>';
                    rowdata += '<tr><th style="font-weight:bold;text-align:center;" colspan=' + _columnno + ' >' + _supplierArray[row_cnt]["_supplierName"] + ' </th></tr>';
                    rowdata += '<tr><th style="font-weight:bold;text-align:right;" colspan=' + _columnno + ' >' + ' Credit Days : ' + _supplierArray[row_cnt]["_creditDays"] + ' </th></tr>';
                    //  rowdata += '</table>';
                    ...
                    const _supplierTransaction = response["data"].filter(d => d.supplierName == _supplierArray[row_cnt]["_supplierName"]);

                    const _dateRecord = _supplierTransaction;
                    var _dateArray = [];
                    ...
                    for (var row_cnt_date = 0; row_cnt_date < _dateArray.length; row_cnt_date++) {
                        ...
                        const _transaction = response["data"].filter(d => d.supplierName == _supplierArray[row_cnt]["_supplierName"] && d.date == _dateArray[row_cnt_date]["_date"]);
                ...
                rowdata += '<tr style="font-weight:bold;">' +
                    ...
                    '<th>' + accounting.format(_Column0Total, 0) + ' </th>' +
