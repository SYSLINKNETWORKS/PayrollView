...
function fillBill() {
    $("#txt_bill").select2({
        ...
        ajax: { // instead of writing the function to execute the request we use Select2's convenient helper
            url: ApiForm + '/LOVServicesPurchase/GetBill',
            type: "Get",
            dataType: 'json',
        ...
        initSelection: function (element, callback) {
            var data = { "id": _Bill_Id, "text": _Bill_Name };
            callback(data);
        },
...
$('#txt_bill').on("select2-selected", function (e) {
    $("#txt_bilno").val($("#txt_bill").select2('data').BillNo);
    $("#txt_rat").val($("#txt_bill").select2('data').Rate);
    $('#txt_cur').val($("#txt_bill").select2('data').CurrencyName);
    $('#txt_sup').val($("#txt_bill").select2('data').SupplierName);
    $('#txt_bildat').val(moment($("#txt_bill").select2('data').BillDate).format("DD/MMM/YYYY"));
});
...
function fillItems() {
    $("#txt_itm").select2({
        ...
        //        allowClear: true,
        ajax: { // instead of writing the function to execute the request we use Select2's convenient helper
            url: ApiForm + '/LOVServicesPurchase/BillDetailByBillId',
            type: "Get",
            contentType: "application/json",
            dataType: "json",
            ...
            transport: function (params) {
                params.beforeSend = function (request) {
                    request.setRequestHeader("Authorization", 'Bearer ' + strkey);
                    request.setRequestHeader("billId", $("#txt_bill").select2('data').id);
                };
...
$('table').on('click', '.btn-edit', function (e) {
    ...
    if (_Stage != "Open") {
        Swal.fire({
            title: "Debit note has been " + _Stage,
            icon: 'warning',
            showConfirmButton: true,
            showClass: {
                popup: 'animated fadeInDown faster'
    ...
    Swal.fire({
        ...
        if (result.value) {
            $.ajax({
                url: ApiForm + '/DebitNote/GetDebitNoteById',
                type: "Get",
                contentType: "application/json",
                dataType: "json",
                beforeSend: function (xhr) {
                ...
                success: function (response) {
                    if (response.statusCode == 200) {
                        ...
                        $("#txt_bilno").val(response["data"]["billNo"]);
                        $("#txt_rat").val(response["data"]["rate"]);
                        $('#txt_cur').val(response["data"]["currencyName"]);
                        $('#txt_sup').val(response["data"]["supplierName"]);
                        $("#txt_bildat").find("input").val(moment(response["data"]["billDate"]).format("DD/MMM/YYYY"));
                        ...
                        _Bill_Id = response["data"]["billMasterId"];
                        _Bill_Name = response["data"]["billName"];
                        $('#txt_bill').val(_Bill_Id); // Select the option with a value of '1'
                        $('#txt_bill').trigger('change'); // Notify any JS components that the value changed



                        if (response["data"]["debitNoteDetailByIdViewModel"] != null) {
                            var _DetailRecord = response["data"]["debitNoteDetailByIdViewModel"];
                            ...
                            for (var user_row_cnt = 0; user_row_cnt < _DetailRecord.length; user_row_cnt++) {
                                ...
                                var _Row = '<tr>' +
                                    ...
                                    '<td>' + accounting.format(_DetailRecord[user_row_cnt]["billQty"], 0) + '</td>' +
...
function ckvalidation() {
    ...
    if (Boolean(ck)) {
        ...
        _cre = JSON.stringify({
            ...
            "category": ddl_category.val(),
            "remarks": txtrmk.val(),
            "debitNoteDetailViewModels": detail_record,
            "type": "U",
            "menu_Id": _menuid
        });
    }
...
function AddRecord() {
    ...
    var _GSTAmount = _Amount * ($("#txt_itm").select2('data').GSTPer / 100);
    var _NetAmount = _Amount + _GSTAmount;
    var productItem = '<tr>' +
        '<td><a data-itemId="0" href="#" class="deleteItem">Remove</a></td>' +
    ...
    _Amount = 0; _DiscountAmount = 0; _GSTAmount = 0; _NetAmount = 0;
...
function calculationMaster() {
    ...
    if (parseFloat(disper) > 0) {
        ...
        $("#txt_disamt").val(accounting.format(disamt.toFixed(0), 0))
    }
...
$('table').on('click', '.btn-print', function (e) {
    ...
    Swal.fire({
        title: "Are sure wants to print Debit Note # " + _no + "?",
        icon: 'warning',
        showCancelButton: true,
        confirmButtonColor: '#5cb85c',
        cancelButtonColor: '#d33',
