...
function printReport() {
    ...
    detailsTable.empty();
    $.ajax({
        url: ApiForm + '/v1/Report/GetIssuanceInventory',
        type: "Post",
        ...
        success: function (response) {
            if (response.statusCode == 200) {
                ...
                const qoutno_1 = response["data"]["issuanceInventoryMasters"];
                ...
                for (var row_cnt = 0; row_cnt < qouno.length; row_cnt++) {
                    const _transaction = response["data"]["issuanceInventoryMasters"].filter(d => d.id == qouno[row_cnt]["_id"]);
                    var _columnno = 11;
                    ...
                    rowdata += '</thead>';
                    var Detailsrecord = _transaction[0]["issuanceInventoryDetails"];
