        ...
        success: function (response) {
            if (response.statusCode == 200) {
                ...
                for (var row_cnt_item = 0; row_cnt_item < uniqueItem.length; row_cnt_item++) {
                    ...
                    var rowdata_1 = '<tr>' +
                        '<td colspan=8 style="font-weight:bold;text-align:center;">Item : ' + _transaction[0]["itemBrandName"] + ' - ' + _transaction[0]["itemName"] + '</td></tr>';
                    detailsTableBody.append(rowdata_1);


                    //Opening Start
                    var bal = 0, amt = 0, receive_subtotal = 0, issue_subtotal = 0, amount_subtotal = 0;
                    ...
                    for (var titmid_cnt = 0; titmid_cnt < StockLedger_O.length; titmid_cnt++) {
                        ...
                        amt += Number(StockLedger_O[titmid_cnt]["amount"]);
                    }
                    ...
                    var rowdata = '<tr>' +
                        ...
                        '<td style="text-align:right;"></td>' +
                        '<td style="text-align:right;">' + accounting.format(amt, 0) + '</td>' +
                        '</tr>';
                    ...
                    for (var titmid_cnt = 0; titmid_cnt < StockLedger_T.length; titmid_cnt++) {
                        ...
                        amount_subtotal += Number(StockLedger_T[titmid_cnt]["amount"]);
                        ...
                        var rowdata = '<tr ' + styleformat + '>' +
                            ...
                            '<td style="text-align:right;">' + StockLedger_T[titmid_cnt]["received"] + '</td>' +
                            '<td style="text-align:right;">' + StockLedger_T[titmid_cnt]["issued"] + '</td>' +
                            '<td style="text-align:right;">' + accounting.format(bal, 0) + '</td>' +
                            '<td style="text-align:right;">' + accounting.format(StockLedger_T[titmid_cnt]["rate"], 2) + '</td>' +
                            '<td style="text-align:right;">' + accounting.format(amount_subtotal, 2) + '</td>' +
                            '</tr>';
                        detailsTableBody.append(rowdata);
                    ...
                    var rowdata_subtotal = '<tr style="font-weight:bold;text-align:right;"><td  colspan=3>Total :</td><td>' + parseFloat(receive_subtotal).toFixed(0) + '</td><td>' + parseFloat(issue_subtotal).toFixed(0) + '</td><td></td><td></td><td>' + parseFloat(amount_subtotal).toFixed(2) + '</td></tr>';
