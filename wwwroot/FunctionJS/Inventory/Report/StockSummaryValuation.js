    ...
    $.ajax({
        ...
        success: function (response) {
            if (response.statusCode == 200) {
                ...
                $('#lbl_head').html("Stock Summary With Valuation <br/>As on " + moment(response.data.dateTo).format("DD-MMM-YYYY"));
                ...
                for (var row_cnt_cat = 0; row_cnt_cat < uniqueCategory.length; row_cnt_cat++) {
                    ...
                    for (var row_cnt_subcat = 0; row_cnt_subcat < uniqueSubCategory.length; row_cnt_subcat++) {
                        ...
                        for (var row_cnt = 0; row_cnt < uniqueBrand.length; row_cnt++) {
                            ...
                            var sno = 0, bd_qty = 0,bd_amount = 0;
                            for (var row_cnt_1 = 0; row_cnt_1 < _transaction.length; row_cnt_1++) {
                                ...
                                var _Item_Amount = _transaction[row_cnt_1]["amount"];
                                ...
                                var rowdata = '<tr>' +
                                    ...
                                    '<td colspan=3><a href="javascript:void(0)" onclick=window.open("' + apiUrl_View + '/Inventory/Report/StockLedgerValuation?S=' + sessid_Item + '","_blank")>' + _transaction[row_cnt_1]["itemName"] + '</a></td>' +
                                    ...
                                    '<td style="text-align:center;' + _style + '">' + accounting.format(_transaction[row_cnt_1]["rate"],2) + '</td>' +
                                    '<td style="text-align:center;' + _style + '">' + accounting.format(_Item_Amount,2) + '</td>' +
                                    '</tr>';

                                detailsTableBody.append(rowdata);
                                bd_qty += parseFloat(_transaction[row_cnt_1]["quantity"]);
                                bd_amount += parseFloat(_transaction[row_cnt_1]["amount"]);
                            ...
                            var subtotal = '';
                            subtotal = '<tr style="text-align:right; font-weight:bold"><td colspan=5>Sub-Total : </td><td style="text-align:center;">' + accounting.format(bd_qty,0) + '</td><td></td><td style="text-align:center;">' + accounting.format(bd_amount,2) + '</td></tr>';
                ...
                var total = '';
                total = '<tr style="text-align:right; font-weight:bold"><td colspan=5>Total : </td><td style="text-align:center;">' + accounting.format(qty,0) + '</td><td></td><td style="text-align:center;">' + accounting.format(amt,2) + '</td></tr>';
