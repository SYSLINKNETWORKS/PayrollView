...
var ComponentsDropdowns = function () {
    var handleSelect2 = function () {
        FillItemCat();   //Fill Select 2 of ItemCategory
    }
    return {
...
$('#txt_itmcat').on("select2-selected", function (e) {
    _ItemCat_Id = $("#txt_itmcat").select2('data').id;
});
...
$('table').on('click', '.btn-edit', function (e) {
    ...
    Swal.fire({
        ...
        if (result.value) {
            $.ajax({
                url: ApiForm + '/Warehouse/GetWarehouseById',
                type: "Get",
                contentType: "application/json",
                dataType: "json",
                beforeSend: function (xhr) {
                ...
                success: function (response) {
                    ...
                    if (response.statusCode == 200) {
                        ...
                        _ItemCat_Id = response["data"]["itemCategoryId"];
                        _ItemCat_Name = response["data"]["itemCategoryName"];
                        $('#txt_itmcat').val(_ItemCat_Id); // Select the option with a value of '1'
                        $('#txt_itmcat').trigger('change'); // Notify any JS components that the value changed
...
function ckvalidation() {
    ...
    if (Boolean(ck)) {
        ...
        _cre = JSON.stringify({
            "id": txtid,
            "name": txtname.val(),
            "itemCategoryId": txtitemCatId.val(),
            "active":ck_active,
            "type": "U",
            "menu_Id": _menuid
        });
