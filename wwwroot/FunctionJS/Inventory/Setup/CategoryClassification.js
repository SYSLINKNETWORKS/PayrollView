...
function Onload() {
    var tbl_row_cnt = 1;
    $.ajax({
        url: ApiForm + '/ItemCategoryClassification',
        type: "Get",
...
$('table').on('click', '.btn-edit', function (e) {
    ...
    Swal.fire({
        ...
        if (result.value) {
            $.ajax({
                url: ApiForm + '/ItemCategoryClassification/GetItemCategoryClassificationById',
                type: "Get",
                contentType: "application/json",
                dataType: "json",
                beforeSend: function (xhr) {
                ...
                success: function (response) {
                    ...
                    if (response.statusCode == 200) {
                        ...
                        _ItemCategory_Id = response["data"]["itemCategoryId"];
                        _ItemCategory_Name = response["data"]["itemCategoryName"];
                        $('#txt_itemcategory').val(_ItemCategory_Id); // Select the option with a value of '1'
                        $('#txt_itemcategory').trigger('change'); // Notify any JS components that the value changed


                        _Account_No = response["data"]["accountNo"];
                        _Account_Name = response["data"]["accountName"];
                        $('#txt_account').val(_Account_No); // Select the option with a value of '1'
...
function ckvalidation() {
    ...
    var ddlcattype = $('#S_type');
    var txtaccount = $('#txt_account');
    var txtcategory = $('#txt_itemcategory');
    ...
    else if (ddlcattype.val() == "0") {
        ck = 1;
        _Error = 'Please Select Category type';
        ddlcattype.focus();
    }
    ...
    if (Boolean(ck)) {
        ...
        _cre = JSON.stringify({
            "ID": txtid,
            "name": txtname.val(),
            "itemCategoryId": txtcategory.val(),
...
function FillAccount() {
    $("#txt_account").select2({
        ...
        ajax: { // instead of writing the function to execute the request we use Select2's convenient helper
            url: apiUrl + '/api/Accounts/v1/LOVServicesAccounts/GetChartOfAccountDetail',// apiUrl + '/SetupCombo/GetBankType',
            type: "Get",
            contentType: "application/json",
            dataType: "json",
...
function FillItemCategory() {
    $("#txt_itemcategory").select2({
        ...
        allowClear: true,
        ajax: { // instead of writing the function to execute the request we use Select2's convenient helper
            url: ApiForm + '/LOVServicesItem/GetItemCatgoryUserWise',
            type: "Get",
            contentType: "application/json",
            dataType: "json",
            transport: function (params) {
        ...
        initSelection: function (element, callback) {
            var data = { "id": _ItemCategory_Id, "text": _ItemCategory_Name };
            callback(data);
        },
        dropdownCssClass: "bigdrop", // apply css that makes the dropdown taller
        escapeMarkup: function (m) {
