...
$(document).ready(function () {
    ...
    $('#ck_act').iCheck('uncheck'); //To check the radio button

    $("#ck_inventory").iCheck('Update')[0].checked;
    $('#ck_inventory').iCheck('uncheck'); //To check the radio button

    $("#ck_paper").iCheck('Update')[0].checked;
    $('#ck_paper').iCheck('uncheck'); //To check the radio button
...
function Onload() {
    var tbl_row_cnt = 1;
    $.ajax({
        url: ApiForm + '/ItemSubCategoryMaster',
        type: "Get",
                ...
                if (response["data"] != null) {
                    ...
                    detailsTableBody = $("#Table_View").dataTable({
                        ...
                        columns: [
                            ...
                            { data: 'name' },
                            {
                                data: 'noInventory', render: function (data) {
                                    return data == true ? 'Yes' : 'No'
                                },
                            },
                            {
                                data: 'paper', render: function (data) {
                                    return data == true ? 'Yes' : 'No'
                                },
                            },
                            {
...
$('table').on('click', '.btn-edit', function (e) {
    ...
    Swal.fire({
        ...
        if (result.value) {
            $.ajax({
                url: ApiForm + '/ItemSubCategoryMaster/GetItemSubCategoryMasterById',
                type: "Get",
                contentType: "application/json",
                dataType: "json",
                beforeSend: function (xhr) {
                ...
                success: function (response) {
                    ...
                    if (response.statusCode == 200) {
                        ...
                        if (!response["data"]["noInventory"]) {
                            $('#ck_inventory').iCheck('uncheck');
                        } else { $('#ck_inventory').iCheck('check'); }
                        if (!response["data"]["active"]) {
                            $('#ck_paper').iCheck('uncheck');
                        } else { $('#ck_paper').iCheck('check'); }
...
function ckvalidation() {
    ...
    ck_noInventopry = $("#ck_inventory").iCheck('Update')[0].checked;
    ck_paper = $("#ck_paper").iCheck('Update')[0].checked;
