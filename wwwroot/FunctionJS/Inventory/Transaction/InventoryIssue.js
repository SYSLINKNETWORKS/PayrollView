...
$('table').on('click', '.btn-edit', function (e) {
    ...
    Swal.fire({
        ...
        if (result.value) {
            $.ajax({
                url: ApiForm + '/InventoryIssue/GetInventoryIssueById',
                type: "Get",
                contentType: "application/json",
                dataType: "json",
                beforeSend: function (xhr) {
                ...
                success: function (response) {
                    if (response.statusCode == 200) {
                        ...
                        if (response["data"]["inventoryIssueDetailByIdViewModel"] != null) {
                            var _UserTable = $("#detailsTable tbody");
                            _UserTable.empty();
                            for (var user_row_cnt = 0; user_row_cnt < response["data"]["inventoryIssueDetailByIdViewModel"].length; user_row_cnt++) {
                                _RequistionId = response["data"]["inventoryIssueDetailByIdViewModel"][user_row_cnt]["inventoryRequisitionMasterId"];
                                _ItemId = response["data"]["inventoryIssueDetailByIdViewModel"][user_row_cnt]["itemId"];
                                _ItemName = response["data"]["inventoryIssueDetailByIdViewModel"][user_row_cnt]["itemName"];

                                var whid = response["data"]["inventoryIssueDetailByIdViewModel"][user_row_cnt]["warehouseId"];
                                var whName = response["data"]["inventoryIssueDetailByIdViewModel"][user_row_cnt]["warehouseName"];

                                var _qty = response["data"]["inventoryIssueDetailByIdViewModel"][user_row_cnt]["qty"];
                                var _stk = response["data"]["inventoryIssueDetailByIdViewModel"][user_row_cnt]["stock"];
                                var _issue = response["data"]["inventoryIssueDetailByIdViewModel"][user_row_cnt]["issue"];

                                var _Row = '<tr>' +
                                    '<td><a data-itemId="0" href="#" class="deleteItem">Remove</a></td>' +
                                    '<td hidden>' + _RequistionId + '</td>' +
                                    '<td hidden>' + _ItemId + '</td>' +
                                    '<td>' + _ItemName + '</td>' +
                                    '<td hidden>' + whid + '</td>' +
                                    '<td>' + whName + '</td>' +
                                    '<td>' + accounting.format(_qty, 0) + '</td>' +
                                    '<td>' + _stk + '</td>' +
                                    '<td><input type="text"  class="form-control" style="width:70px"  value=' + _issue + '></td>';
...
//Fill InventoryRequisition

function FillInventoryRequisition() {
    $("#txt_req").select2({
        ...
        ajax: { // instead of writing the function to execute the request we use Select2's convenient helper
            url: ApiForm + '/LOVServicesItem/GetInventoryRequisition',
            type: "Get",
            contentType: "application/json",
            dataType: "json",
...
function ckvalidation() {
    ...
    for (var i = 0; i < rows_create.length; i++) {
        ...
        var RequisitionId = $(columns[1]).html().trim();
        var ItemId = $(columns[2]).html().trim();
        var whId = $(columns[4]).html().trim();
        var _qty = accounting.unformat($(columns[6]).html(), 0);
        var _stock = accounting.format($(columns[7]).html(), 0);
        var _issue = parseFloat($(columns[8]).find("input[type='text']").val());
        ...
        if (_issue > _qty) {
            _Error = 'issuence not greater then Inventory requisition';
            ck = true;
        }
        ...
        detail_record.push({
            "itemId": ItemId,
            "warehouseId": whId,
            "InventoryRequisitionMasterId": RequisitionId,
            "issue": _issue,
        })
    }
    ...
    if (Boolean(ck)) {
        ...
        _cre = JSON.stringify({
            "ID": txtid,
            "date": date1,
            "InventoryIssueDetailViewModels": detail_record,
            "type": "U",
            "menu_Id": _menuid
        });
...
function AddRequisition() {
    ...
    $.ajax({
        url: ApiForm + '/LOVServicesItem/GetInventoryRequisitionDetailById',
        type: "Get",
        contentType: "application/json",
        dataType: "json",
        beforeSend: function (xhr) {
            ...
            if (response.statusCode == 200) {
                ...
                for (var row_cnt = 0; row_cnt < _DetailRecord.length; row_cnt++) {
                    ...
                    productItem += '<tr>' +
                        ...
                        '<td >' + accounting.format(_DetailRecord[row_cnt]["inventoryRequisition"], 0) + '</td>' +
