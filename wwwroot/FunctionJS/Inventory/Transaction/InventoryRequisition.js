...
var ComponentsDropdowns = function () {
    var handleSelect2 = function () {
        FILLDepartment(); //Fill Select 2 of Inventory Department    
        //Fill Select 2 of SalesOrder
...
function Onload() {
    var tbl_row_cnt = 1;
    $.ajax({
        url: ApiForm + '/InventoryRequisition',
        type: "Get",
        contentType: "application/json",
        dataType: "json",
...
$('table').on('click', '.btn-edit', function (e) {
    ...
    Swal.fire({
        ...
        if (result.value) {
            $.ajax({
                url: ApiForm + '/InventoryRequisition/GetInventoryRequisitionById',
                type: "Get",
                contentType: "application/json",
                dataType: "json",
                beforeSend: function (xhr) {
                ...
                success: function (response) {
                    if (response.statusCode == 200) {
                        ...
                        $('#txt_dpt').val(_Department_Id); // Select the option with a value of '1'
                        $('#txt_dpt').trigger('change'); // Notify any JS components that the value changed


                        if (response["data"]["inventoryRequisitionDetailByIdViewModel"] != null) {
                            var _UserTable = $("#detailsTable tbody");
                            _UserTable.empty();
                            for (var user_row_cnt = 0; user_row_cnt < response["data"]["inventoryRequisitionDetailByIdViewModel"].length; user_row_cnt++) {
                                _ItemId = response["data"]["inventoryRequisitionDetailByIdViewModel"][user_row_cnt]["itemId"];
                                _ItemName = response["data"]["inventoryRequisitionDetailByIdViewModel"][user_row_cnt]["itemName"];
                                _qty = response["data"]["inventoryRequisitionDetailByIdViewModel"][user_row_cnt]["qty"];
                                _scalename = response["data"]["inventoryRequisitionDetailByIdViewModel"][user_row_cnt]["uom"];
...
function ckvalidation() {
    ...
    for (var i = 0; i < rows_create.length; i++) {
        ...
        var ItemId = $(columns[1]).html().trim();
        var qty = parseFloat($(columns[3]).html().trim());
    ...
    if (Boolean(ck)) {
        ...
        _cre = JSON.stringify({
            "ID": txtid,
            "date": txtdate,
            "DepartmentId": txtdpt.val(),
            "InventoryRequisitionDetailViewModels": detail_record,
            "type": "U",
            "menu_Id": _menuid
        });
...
function FILLDepartment() {
    $("#txt_dpt").select2({
        ...
        initSelection: function (element, callback) {
            console.log(_Department_Id, _Department_Name)
